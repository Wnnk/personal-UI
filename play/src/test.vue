...
const stopAnimate = () =>{
  ...
  if(mustIndex.value){
    ...
    console.log('角度范围',startAngle.value, endAngle.value)
...
const subSpeed=()=>{
  ...
  if(mustIndex.value){
    if(!(angle.value % (2*Math.PI)>startAngle.value
      ...
      console.log('减速到0:', speed.value, startAngle.value, endAngle.value)
      cancelAnimationFrame(animateFlag.value)
      cancelAnimationFrame(speedFlag.value)
    }
    else{
      console.log('范围停止:',speed.value, startAngle.value, endAngle.value)
      cancelAnimationFrame(animateFlag.value)
      cancelAnimationFrame(speedFlag.value)
    }
  }else{
    if(speed.value<0){
      console.log('没设置target,减速到0')
      cancelAnimationFrame(animateFlag.value)
      cancelAnimationFrame(speedFlag.value)
      // console.log("6");
    }else{
      console.log('没设置target,减速')
      speedFlag.value = requestAnimationFrame(subSpeed);
    }
  }
}
...
const animate=async()=>{
  ...
  if(backImageOption.type == 'image'){
    ...
    for (const [index,item] of ballOptions.entries()){
        ...
        console.log('初始化角度',startAngle, endAngle, angle.value)
        // 初始化角度 0 1.0471975511965976 0.1
        // test.vue:317 初始化角度 1.0471975511965976 2.0943951023931953 0.1
        // test.vue:317 初始化角度 2.0943951023931953 3.141592653589793 0.1
        // test.vue:317 初始化角度 3.141592653589793 4.1887902047863905 0.1
        // test.vue:317 初始化角度 4.1887902047863905 5.235987755982989 0.1
        // test.vue:317 初始化角度 5.235987755982989 0 0.1
        await drawArc(300,300,200,startAngle+angle.value,endAngle+angle.value,item.color,item)
      }
  }
